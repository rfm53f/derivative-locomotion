...
vr::EVRInitError MyDeviceProvider::Init( vr::IVRDriverContext *pDriverContext )
{
	...
	my_controller_device_ = std::make_unique< MyControllerDeviceDriver >();
	...
	if ( !vr::VRServerDriverHost()->TrackedDeviceAdded( my_controller_device_->MyGetSerialNumber().c_str(), vr::TrackedDeviceClass_Controller, my_controller_device_.get() ) )
	{
...
void MyDeviceProvider::RunFrame()
{
	...
	if ( my_controller_device_ != nullptr )
	{
		my_controller_device_->MyRunFrame();
	}
	...
	while ( vr::VRServerDriverHost()->PollNextEvent( &vrevent, sizeof( vr::VREvent_t ) ) )
	{
		if ( my_controller_device_ != nullptr )
		{
			my_controller_device_->MyProcessEvent( vrevent );
		}
	}
}
